...
close_file_entry (NODE_T * p, int k)
{
  if (k >= 0 && k < MAX_OPEN_FILES) {
    ...
    if (IS_OPEN (fe)) {
/* Close the file */
      if (FD (fe) != A68_NO_FILENO && _close (FD (fe)) == -1) {
        init_file_entry (k);
...
#endif
  if (FD (file) != A68_NO_FILENO && _close(FD (file)) == -1) {
...
/* Now set */
    ...
    __off_t curpos = _lseek (FD (file), 0, SEEK_CUR);
    __off_t maxpos = _lseek(FD (file), 0, SEEK_END);
    __off_t res = _lseek(FD (file), curpos, SEEK_SET);
/* Circumvent buffering problems */
    int reserve = get_transput_buffer_index (TRANSPUT_BUFFER (file));
    curpos -= (__off_t) reserve;
    res = _lseek(FD (file), -reserve, SEEK_CUR);
...
/* Now set */
    ...
    if (curpos < 0 || curpos >= maxpos) {
      ...
      PUSH_PRIMITIVE (p, (int)_lseek(FD (file), 0, SEEK_CUR), A68_INT);
    } else {
      res = _lseek(FD (file), curpos, SEEK_SET);
      if (res == -1 || errno != 0) {
...
*/
  ...
  for (i = 0; prefix[i] != NO_TEXT; i++) {
    for (k = 0; k < TRIALS && good_file == A68_FALSE; k++) {
      ...
      RESET_ERRNO;
      fd = _open (tfilename, flags | O_EXCL, permissions);
...
/* No identification, so generate a unique identification. */
    if (reading) {
      ...
      FD (file) = _open(tfilename, flags, permissions);
...
/* Establishing requires that the file does not exist */
    ...
    FD (file) = _open(filename, flags, permissions);
    TRANSPUT_BUFFER (file) = get_unblocked_transput_buffer (p);
    reset_transput_buffer (TRANSPUT_BUFFER (file));
    END_OF_FILE (file) = A68_FALSE;
...
genie_cd (NODE_T * p)
{
  ...
  if (buffer == NO_TEXT) {
    ...
    int rc = _chdir (a_to_c_string (p, buffer, dir));
    if (rc == 0) {
      PUSH_PRIMITIVE (p, _chdir (a_to_c_string (p, buffer, dir)), A68_INT);
    } else {
      diagnostic_node (A68_RUNTIME_ERROR, p, ERROR_FILE_ACCESS);
      exit_genie (p, A68_RUNTIME_ERROR);
    }
